...
## 7. 보안 및 정책 준수
- **하드코딩 금지:** 소스 코드 내에 API 비밀번호, 관리자 키, 기본 이미지 URL 등 민감하거나 특정 값을 하드코딩하지 마십시오. 모든 입력 폼의 기본값은 비워두거나 `null`로 유지해야 합니다.
- **민감 정보 보호:** 관리자 비밀번호 입력 필드에는 비밀번호 마스킹을 적용하고, 자동 완성 기능을 방지하십시오.
